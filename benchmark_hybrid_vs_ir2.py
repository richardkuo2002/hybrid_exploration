...
def run_benchmark():
    # Configuration matching IR2 Complex benchmark
    N_RUNS = 20
    AGENT_NUM = 5
    MAP_TYPE = "ir2"
    OUTPUT_DIR = "results_comparison"
    
    # Tasks: Run indices 0-19, Map indices 0-19 (assuming sorted order matches)
    # IR2 uses map_index = global_step. Env sorts maps.
    # So map_index 0 corresponds to the first map in the sorted list.
    tasks = []
    for i in range(N_RUNS):
        # args_tuple: (run_index, agent_num, map_index, graph_update_interval, map_type)
        tasks.append((i, AGENT_NUM, i, None, MAP_TYPE))
        
    print(f"Starting Hybrid vs IR2 Benchmark: {N_RUNS} runs, {AGENT_NUM} agents, map_type={MAP_TYPE}")
    ...
    # Sort results by run_index (which equals map_index here)
    ...
    filename_base = f"hybrid_vs_ir2_{timestamp}"
