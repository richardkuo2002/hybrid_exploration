...
class Robot():
    ...
    def update_local_awareness(self, real_map, all_robots, server, find_frontier_func, merge_maps_func):
        ...
        if self.is_in_server_range and not was_in_range:
             ...
             self.return_replan_attempts = 0
             self.return_fail_cooldown = 0
             logger.debug(f"Robot {self.robot_id} re-entered server range. State reset.") # 使用 logger
        ...
        try: # 增加錯誤處理
            node_coords, graph, node_utility, guidepost = self.graph_generator.update_graph(
                self.local_map, new_frontiers, self.frontiers, self.position, # 傳入 all_robot_positions? Server 會做
            )
            ...
            self.guidepost = guidepost
             ...
             logger.error(f"Robot {self.robot_id} failed update_graph: {e}")
             # 可以選擇保持舊圖，或清空
             self.node_coords = None # 清空以標示錯誤
             self.local_map_graph = None
    ...
    def decide_next_target(self, all_robots):
        ...
        if self.is_returning:
            if len(self.planned_path) < 1:
                if self.return_replan_attempts < MAX_RETURN_REPLAN_ATTEMPTS:
                    logger.debug(f"[R{self.robot_id} Decide] Returning path empty. Re-planning (Attempt {self.return_replan_attempts + 1}).")
                    ...
                    if not self.planned_path or (len(self.planned_path) == 1 and np.array_equal(self.planned_path[0], self.position)):
                         ...
                         self.planned_path = [self.position] # Stay
                else:
                    logger.warning(f"[R{self.robot_id} Decide] Failed return plan after {MAX_RETURN_REPLAN_ATTEMPTS} attempts. Aborting return.")
                    self.is_returning = False
                    self.return_replan_attempts = 0
                    self.return_fail_cooldown = RETURN_FAIL_COOLDOWN_STEPS
            else:
                 logger.debug(f"[R{self.robot_id} Decide] Continuing return path.")
                 return # Continue current path

        # --- 處理冷卻期 ---
        if self.return_fail_cooldown > 0:
            logger.debug(f"[R{self.robot_id} Decide] In cooldown ({self.return_fail_cooldown} left). Forcing local.")
            ...
            self.is_returning = False; self.target_gived_by_server = False
            return
        ...
        if should_return_criteria or local_target_too_far:
            ...
            if planned_return_path and not (len(planned_return_path) == 1 and np.array_equal(planned_return_path[0], self.position)):
                ...
                self.is_returning = True; self.return_replan_attempts = 0; self.return_fail_cooldown = 0
            ...
            target_pos = target_pos_local
            self.is_returning = False; self.return_fail_cooldown = 0
            self.planned_path = self._plan_local_path(target_pos)
            if not self.planned_path: self.planned_path = [self.position]
            decision_reason = "Local Explore"
    ...
    def move_one_step(self, all_robots):
        ...
        if np.array_equal(next_step, self.position):
            if len(self.planned_path) > 1:
                ...
                logger.debug(f"[R{self.robot_id} Move] Path start=curr pos, popped. Next: {next_step}")
            else:
                logger.debug(f"[R{self.robot_id} Move] Reached target or stuck at {self.position}")
        ...
        if is_blocked:
            logger.debug(f"[R{self.robot_id} Move] Blocked by R{blocker_id} @ {all_robots[blocker_id].position}. NextStep: {next_step}. Stay: {self.stay_count}")
    ...
    def _select_node(self, all_robots):
        ...
        if not hasattr(self.graph_generator, 'candidates_utility') or \
           ...
           len(self.graph_generator.candidates_utility) != num_candidates:
            logger.error(f"[R{self.robot_id} SelectNode] Mismatch or missing candidates_utility!")
            return self.position, 0, 0
    ...
    def _plan_local_path(self, target, avoid_pos=None):
        ...
        if coords is None or len(coords) == 0 or graph is None or not hasattr(graph, 'nodes') or not hasattr(graph, 'edges'):
             logger.warning(f"[R{self.robot_id} PlanLocal] Invalid graph/nodes. Target:{target}")
             return [current]
