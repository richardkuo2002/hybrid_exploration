...
class Robot:
    def __init__(
        ...
        try:
            ...
            logger.debug(
                f"Robot init: initial rebuild_graph_structure failed for R{getattr(self,'robot_id',-1)}",
                exc_info=True,
            )
    ...
    def decide_next_target(self, all_robots: List["Robot"]) -> None:
        ...
        if self.is_returning:
            if len(self.planned_path) < 1:
                if self.return_replan_attempts < MAX_RETURN_REPLAN_ATTEMPTS:
                    logger.debug(
                        f"[R{self.robot_id} Decide] Return path empty. Replan attempt {self.return_replan_attempts + 1}."
                    )
                    ...
                    if not self.planned_path or (
                        len(self.planned_path) == 1
                        and np.array_equal(self.planned_path[0], self.position)
                    ):
                        logger.debug(
                            f"[R{self.robot_id} Decide] Replan attempt {self.return_replan_attempts} failed."
                        )
                        self.planned_path = [self.position]
                else:
                    logger.warning(
                        f"[R{self.robot_id} Decide] Failed return plan after {MAX_RETURN_REPLAN_ATTEMPTS} attempts. Aborting."
                    )
                    self.is_returning = False
                    self.return_replan_attempts = 0
        ...
        if self.return_fail_cooldown > 0:
            logger.debug(
                f"[R{self.robot_id} Decide] In cooldown ({self.return_fail_cooldown} left). Force local."
            )
        ...
        if (
            ...
            and len(self.graph_generator.candidates_utility) > 0
        ):
        ...
        local_target_too_far = (
            min_valid_dists > (self.sensor_range * 1.5) and self.out_range_step > 5
        )
        logger.debug(
            f"[R{self.robot_id} Decide] Checks: Stag={is_stagnated}({recent_info_gain}<{MIN_INFO_GAIN_THRESHOLD}), Dep={is_depleted}({max_local_utility}<{LOCAL_UTILITY_THRESHOLD}), Time={is_timeout}({self.out_range_step}>{OUT_RANGE_STEP}), Far={local_target_too_far}({min_valid_dists:.1f})"
        )
        ...
        if should_return_criteria or local_target_too_far:
            ...
            if planned_return_path and not (
                ...
                decision_reason = (
                    "Return (Criteria)"
                    if should_return_criteria
                    else "Return (Too Far)"
                )
                logger.debug(
                    f"[R{self.robot_id} Decide] START RETURN ({decision_reason}). PathLen={len(self.planned_path)}"
                )
            else:
                logger.warning(
                    f"[R{self.robot_id} Decide] Wanted return ({'Criteria' if should_return_criteria else 'TooFar'}), but path failed. Trigger cooldown & local."
                )
                self.return_fail_cooldown = RETURN_FAIL_COOLDOWN_STEPS
        ...
        logger.debug(
            f"[R{self.robot_id} Decide] Final Target: {self.target_pos}, Reason: {decision_reason}, PathLen: {path_len}"
        )
    ...
    def move_one_step(self, all_robots: List["Robot"]) -> None:
        ...
        """
        logger.debug(
            f"[R{self.robot_id} Move] Pos:{self.position}, Target:{self.target_pos}, Return:{self.is_returning}, PathLen:{len(self.planned_path)}"
        )
        if len(self.planned_path) < 1:
        ...
        if np.array_equal(next_step, self.position):
            if len(self.planned_path) > 1:
                ...
                next_step = self.planned_path[0]
                logger.debug(
                    f"[R{self.robot_id} Move] Path start=curr, popped. Next:{next_step}"
                )
            else:
        ...
        if is_blocked:
            ...
            if self.stay_count > MAX_STAY_COUNT:
                ...
                if len(self.movement_history) > 1:
                    ...
                    if can_move_back:
                        ...
                        self.stay_count = 0
                        logger.debug(
                            f"[R{self.robot_id} Move] Moved back to {self.position}. Path cleared."
                        )
            ...
            if len(self.planned_path) > 0:
                ...
                if (
                    np.array_equal(popped_step, self.position)
                    and len(self.planned_path) > 0
                ):
    ...
    def _select_node(self, all_robots: List["Robot"]) -> Tuple[np.ndarray, int, float]:
        ...
        """
        if (
            not hasattr(self.graph_generator, "target_candidates")
            or self.graph_generator.target_candidates is None
        ):
            logger.debug(
                f"[R{self.robot_id} SelectNode] graph_generator.target_candidates is None!"
            )
            # Diagnostic: report local graph status
            try:
                gcoords_len = (
                    len(self.node_coords) if self.node_coords is not None else 0
                )
            except Exception:
                gcoords_len = -1
            try:
                local_graph_empty = (
                    isinstance(self.local_map_graph, dict)
                    and len(self.local_map_graph) == 0
                )
            except Exception:
                local_graph_empty = True
            logger.debug(
                f"[Diag robot R{self.robot_id}] node_coords_len={gcoords_len} local_map_graph_empty={local_graph_empty}"
            )
        ...
        if num_candidates == 0:
            ...
            try:
                gcoords_len = (
                    len(self.graph_generator.node_coords)
                    if hasattr(self.graph_generator, "node_coords")
                    and self.graph_generator.node_coords is not None
                    else 0
                )
            except Exception:
                gcoords_len = -1
            try:
                targ_len = (
                    len(self.graph_generator.target_candidates)
                    if hasattr(self.graph_generator, "target_candidates")
            ...
            try:
                util_len = (
                    len(self.graph_generator.candidates_utility)
                    if hasattr(self.graph_generator, "candidates_utility")
                    and self.graph_generator.candidates_utility is not None
                    else 0
                )
            except Exception:
                util_len = -1
            logger.debug(
                f"[Diag robot R{self.robot_id}] gen_node_coords_len={gcoords_len} gen_target_candidates_len={targ_len} gen_candidates_utility_len={util_len}"
            )
            return self.position, 0, 0
        if (
            not hasattr(self.graph_generator, "candidates_utility")
            or self.graph_generator.candidates_utility is None
            or len(self.graph_generator.candidates_utility) != num_candidates
        ):
            logger.error(
                f"[R{self.robot_id} SelectNode] Mismatch/missing candidates_utility!"
            )
            return self.position, 0, 0
        ...
        try:
            from parameter import (
                SELECTION_PATH_COST_TOPK,
                SELECTION_USE_PATH_COST,
                SELECTION_UTILITY_WEIGHT,
            )
        ...
        try:
            if SELECTION_USE_PATH_COST:
                ...
                topk = (
                    int(SELECTION_PATH_COST_TOPK)
                    if SELECTION_PATH_COST_TOPK is not None
                    else 5
                )
                ...
                for ii, vi in enumerate(topk_indices):
                    ...
                    try:
                        dist, route = self.graph_generator.find_shortest_path(
                            self.position, cand, self.node_coords, None
                        )
                ...
                if np.max(path_costs) - np.min(path_costs) > 0:
                    pc_norm = (path_costs - np.min(path_costs)) / (
                        np.max(path_costs) - np.min(path_costs) + epsilon
                    )
                else:
                    pc_norm = np.zeros_like(path_costs)
                for idx_local, pcn in zip(topk_indices, pc_norm):
                    # 用 1 - pcn 作為距離部分的替代（距離小 => 高分）
                    scores[idx_local] = w_u * util_norm[idx_local] + (1.0 - w_u) * (
                        1.0 - pcn
                    )
        except Exception:
        ...
        logger.debug(
            f"[R{self.robot_id} SelectNode] Cands:{num_candidates}, Valid:{num_valid_after_filter} -> Sel:{selected_coord}, Score:{scores[best_idx_in_valid]:.2f}, Util:{valid_utilities[best_idx_in_valid]}, Dist:{valid_dists[best_idx_in_valid]:.1f}"
        )
        return selected_coord, original_idx, min_valid_dists

    def _plan_local_path(
        self, target: np.ndarray, avoid_pos: Optional[np.ndarray] = None
    ) -> List[np.ndarray]:
        ...
        if (
            ...
            logger.warning(
                f"[R{self.robot_id} PlanLocal] Invalid graph/nodes for target {target}. Coords:{coords is not None}, Graph:{graph_edges is not None}"
            )
            return [current]
        ...
        if coords is not None:
            try:
                ...
                logger.error(
                    f"[R{self.robot_id} PlanLocal] Coords not iterable? Type: {type(coords)}"
                )
                return [current]
        ...
        logger.debug(
            f"[R{self.robot_id} PlanLocal] Target:{target}. A* Result:{'Success' if route is not None else 'Failed!'}, Dist:{dist:.1f}, PathLen:{path_len}"
        )
