...
def measure(n_steps=100, n_agents=3, map_index=0):
    ...
    for step in range(n_steps):
        t0 = time.time()
        done, coverage = server.update_and_assign_tasks(
            env.robot_list, env.real_map, env.find_frontier
        )
        t1 = time.time()
        elapsed = t1 - t0
        ...
        if last is not None:
            ...
            print(
                f"Step {step}: FULL_REBUILD took {last:.3f}s (total step {elapsed:.3f}s) coverage={coverage:.3f}"
            )
        else:
            print(
                f"Step {step}: lightweight (total step {elapsed:.3f}s) coverage={coverage:.3f}"
            )
        if done:
