...
def make_robot(id, pos):
    r = Robot(start_position=np.array(pos), real_map_size=(20,20), resolution=1, k_size=5)
    r.robot_id = id
    return r
...
def test_deep_copy_handoff():
    ...
    assert len(r0.planned_path) == 3, f"r0 planned_path should be length 3, got {len(r0.planned_path)}"
    assert not any((np.array_equal(p, np.array([99,99])) for p in r0.planned_path)), "r0 planned_path was affected by r1 modification (shared reference)"
    assert r0.handoff_cooldown == 3, "r0 handoff_cooldown should be set to HANDOFF_COOLDOWN"
    assert r1.handoff_cooldown == 3, "r1 handoff_cooldown should be set to HANDOFF_COOLDOWN"
